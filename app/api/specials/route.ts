...
export async function GET(request: NextRequest) {
  try {
    ...
    const products  = uniqueIds.length > 0 ? await db.collection('products').find({ _id: { $in: uniqueIds.map(id => new ObjectId(id)) } }).toArray() : [];
    const productMap = Object.fromEntries(products.map(p => [p._id.toString(), p]));

    const enrichedSpecials = specials.map(s => {
      const primaryId = s.productId?.toString() || s.productIds?.[0]?.toString() || s.conditions?.buyProductId?.toString() || null;
...
export async function POST(request: NextRequest) {
  ...
  if ('error' in auth) return NextResponse.json({ error: auth.error }, { status: auth.status });
  if (auth.isSuperAdmin) return NextResponse.json({ error: 'Super admins cannot create specials directly. Please assign to a branch.' }, { status: 403 });
  try {
