...
export async function POST(request: NextRequest) {
  try {
    ...
    // Whether payment is already confirmed at order-creation time.
    // Cash orders are always paid on the spot.
    // Paystack orders start as payment_pending — the webhook handles them.
    const isPaid = body.paymentStatus === 'paid' || body.paymentMethod === 'cash';

    // ── Notify branch pickers — only for paid/cash orders ─────────────────
    // Paystack orders that haven't been confirmed yet must not appear in the
    // picker queue. The webhook fires this notification once charge.success
    // comes in and updates the order to paymentStatus: 'paid'.
    if (isPaid) {
    ...
    // Send immediately so the customer has their order reference regardless
    // of payment method. For Paystack this is the "we received your order"
    // email; a separate status email fires from the webhook on payment success.
    if (customerEmail) {
    ...
    // ── Internal POS receipt — only sent when payment is confirmed ────────
    // Cash: fires now. Paystack: fires from the webhook after charge.success.
    // This prevents staff seeing unpaid orders on the POS inbox.
    if (isPaid) {
