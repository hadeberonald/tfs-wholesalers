...
export async function GET(request: NextRequest) {
  try {
    ...
    const listed       = searchParams.get('listed'); // ← NEW
    ...
    if (listed   === 'true') query.listed   = true; // ← NEW
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existing = await db.collection('categories').findOne({ branchId: adminInfo.branchId, slug });
    if (existing) {
      return NextResponse.json(
