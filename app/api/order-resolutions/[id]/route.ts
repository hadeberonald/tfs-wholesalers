...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!existing) return NextResponse.json({ error: 'Resolution not found' }, { status: 404 });
    ...
    if (body.status === 'resolved') { updateData.resolvedBy = auth.userId; updateData.resolvedAt = new Date(); }

    await db.collection('orderResolutions').updateOne({ _id: new ObjectId(params.id) }, { $set: updateData });
    ...
    if (body.adjustStock && existing.affectedItems?.length > 0) {
      ...
      if (addStockActions.includes(action) || removeStockActions.includes(action)) {
        ...
        for (const item of existing.affectedItems) {
          ...
          if (item.variantId) {
            await db.collection('products').updateOne({ _id: new ObjectId(item.productId.toString()), 'variants._id': item.variantId }, { $inc: { 'variants.$.stockLevel': qty }, $set: { updatedAt: new Date() } });
          } else {
            await db.collection('products').updateOne({ _id: new ObjectId(item.productId.toString()) }, { $inc: { stockLevel: qty }, $set: { updatedAt: new Date() } });
          }
        }
