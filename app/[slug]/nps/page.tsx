...
import {
  CheckCircle, ChevronRight, ChevronDown, User, Phone, Mail,
...
const EMPTY_DATA: SurveyData = {
  overallSatisfaction: '', recommendLikelihood: '', oneImprovement: '',
  metExpectations: '', threeWords: '', easyToFind: '', cleanliness: '',
  checkoutWait: '', greetedByStaff: '', staffFriendliness: '',
  staffRecommendation: '', staffRecommendationDetails: '', foundAllItems: '',
  productQuality: '', promotionsDriven: '', newProductSuggestions: '',
};

// ─── Shared styles ────────────────────────────────────────────────────────────

const INPUT_CLS  = 'w-full border border-gray-200 rounded-2xl px-4 py-3 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-300 focus:border-transparent transition bg-gray-50 focus:bg-white';
const SELECT_CLS = `${INPUT_CLS} appearance-none cursor-pointer pr-10`;

// ─── Pure sub-components (outside main component — no hook calls) ─────────────

function SectionBadge({
  icon: Icon, label, subtitle, iconColor, bg, border,
}: {
...
function SelectField({
  label, value, onChange, options, optional = false,
}: {
...
function TextAreaField({
  label, value, onChange, placeholder, optional = true, rows = 2,
}: {
  label: string; value: string; onChange: (v: string) => void;
...
function NextButton({
  ...
  return (
    <button
      ...
      style={{
        background: !(disabled || loading) ? 'linear-gradient(135deg, #f97316, #ea580c)' : '#d1d5db',
        boxShadow: !(disabled || loading) ? '0 8px 24px rgba(249,115,22,0.35)' : 'none',
      }}
    >
      {loading ? (
    ...
    <div className="flex items-center justify-between mb-5">
      ...
      <div className="flex gap-1.5">
        {STEPS.slice(0, -1).map((s, i) => (
          <div
            key={s}
            className={`rounded-full transition-all duration-300 h-2 ${i <= currentIdx ? 'w-6 bg-brand-orange' : 'w-2 bg-gray-200'}`}
          />
        ))}
      </div>
    </div>
...
export default function NPSSurveyPage() {
  ...
  const [data, setData]         = useState<SurveyData>(EMPTY_DATA);
  const [contact, setContact]   = useState<ContactInfo>({ name: '', phone: '', email: '' });

  const set = (key: keyof SurveyData) => (v: string) =>
    setData((d) => ({ ...d, [key]: v }));
  ...
  const doSubmit = async (withContact: boolean) => {
    if (withContact && contact.phone && !validatePhone(contact.phone)) return;
    setSubmitting(true);
    try {
      await fetch('/api/nps', {
        method: 'POST',
        ...
        <div className="w-full max-w-lg">
          ...
          {step === 'overall' && (
            <div className="bg-white rounded-3xl shadow-xl shadow-orange-100 p-8 border border-orange-100">
              <CardHeader currentIdx={currentIdx} />
              <SectionBadge
                icon={Star}
          ...
          {step === 'store' && (
            <div className="bg-white rounded-3xl shadow-xl shadow-orange-100 p-8 border border-orange-100">
              <CardHeader currentIdx={currentIdx} onBack={() => setStep('overall')} />
              <SectionBadge
                icon={Store}
          ...
          {step === 'staff' && (
            <div className="bg-white rounded-3xl shadow-xl shadow-orange-100 p-8 border border-orange-100">
              <CardHeader currentIdx={currentIdx} onBack={() => setStep('store')} />
              <SectionBadge
                icon={Users}
          ...
          {step === 'products' && (
            <div className="bg-white rounded-3xl shadow-xl shadow-orange-100 p-8 border border-orange-100">
              <CardHeader currentIdx={currentIdx} onBack={() => setStep('staff')} />
              <SectionBadge
                icon={ShoppingCart}
              ...
              <TextAreaField
                ...
                value={data.newProductSuggestions}
                onChange={set('newProductSuggestions')}
                placeholder="e.g. More organic options, a loyalty card, bulk cleaning supplies…"
              />
              <NextButton
                label="Next — Your Details"
                onClick={() => setStep('contact')}
                disabled={!data.foundAllItems || !data.productQuality || !data.promotionsDriven}
              />
            </div>
          )}
          ...
          {step === 'contact' && (
            <div className="bg-white rounded-3xl shadow-xl shadow-orange-100 p-8 border border-orange-100">
              <CardHeader currentIdx={currentIdx} onBack={() => setStep('products')} />

              <div className="text-center mb-6">
                <div className="w-12 h-12 bg-orange-100 rounded-2xl flex items-center justify-center mx-auto mb-3">
                  <User className="w-6 h-6 text-brand-orange" />
                </div>
                <h2 className="text-xl font-black text-gray-900 mb-1">Stay in touch</h2>
              ...
              <div className="mb-4">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`${INPUT_CLS} pl-10 ${phoneError ? 'border-red-300 ring-1 ring-red-300' : ''}`}
                  />
                </div>
              ...
              <p className="text-center text-xs text-gray-400 mt-3">
                Your details are only used to follow up on your feedback.
              </p>
            </div>
          )}
