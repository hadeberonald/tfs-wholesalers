...
export default function SpecialCard({ special }: SpecialCardProps) {
  ...
  // Product is now embedded by the API — no fetch needed
  ...
  const savings = (() => {
    ...
    if (special.type === 'multibuy') {
      return Math.max(
        0,
        product.price * (special.conditions.requiredQuantity || 1) -
          (special.conditions.specialPrice ?? product.price),
      );
    }
    ...
    switch (special.type) {
      case 'buy_x_get_y':
        return `Buy ${special.conditions.buyQuantity}, Get ${special.conditions.getQuantity}${
          special.conditions.getDiscount === 100 ? ' FREE!' : ` ${special.conditions.getDiscount}% off`
        }`;
      case 'multibuy':
  ...
  const handleAddToCart = (e: any) => {
    ...
    Alert.alert(
      ...
      isMultibuy
        ...
        : `${actualQty} ${product.name} added to your cart`,
    );
    setBundleQty(1);
  };

  // ── If no product data came through, render nothing ───────────────────────
        ...
        {inStock ? (
          <View style={shared.cartActions} onStartShouldSetResponder={() => true}>
            <View style={shared.quantityControl}>
              ...
              >
                <Plus
                  color={(bundleQty + 1) * bundleSize > product.stockLevel ? '#d1d5db' : '#6b7280'}
                  size={16}
                />
              </TouchableOpacity>
            </View>
