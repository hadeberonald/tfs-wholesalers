...
function VariantPickerModal({
  ...
  return (
    <>
      ...
      >
        <Text
          style={[pickerStyles.triggerText, selected.outOfStock && pickerStyles.triggerTextMuted]}
          numberOfLines={1}
        >
          {selected.label}
        </Text>
        ...
        <View style={pickerStyles.sheet}>
          ...
          <FlatList
            ...
            renderItem={({ item: opt }) => {
              ...
              return (
                <Pressable
                  onPress={() => { if (!opt.outOfStock) { onChange(opt.value); setOpen(false); } }}
                  style={[
                    pickerStyles.option,
                ...
                >
                  <View style={pickerStyles.optionText}>
                    ...
                    {opt.sublabel
                      ? <Text style={[pickerStyles.optionSub, isSelected && pickerStyles.optionSubSelected]}>{opt.sublabel}</Text>
                      : null}
                    {opt.outOfStock
...
const pickerStyles = StyleSheet.create({
  trigger: {
    ...
    backgroundColor: '#fff', paddingHorizontal: 10, paddingVertical: 6, marginBottom: 6,
  },
...
export default function SpecialCard({ special }: SpecialCardProps) {
  ...
  // Guard: hide card if product is missing or inactive
  ...
  // Fetch add-on product once for conditional_add_on_price
  useEffect(() => {
    if (
      special.type === 'conditional_add_on_price' &&
      special.conditions?.targetProductId
    ) {
  ...
  const selectedVariant = (() => {
    ...
    if ((product.stockLevel ?? 0) === 0) {
      return activeVariants.find((v) => v.stockLevel > 0);
    }
    return undefined;
  })();
  ...
  const savings = (() => {
    if (special.type === 'multibuy') {
      return Math.max(
        0,
        basePrice * (special.conditions.requiredQuantity || 1) -
          (special.conditions.specialPrice ?? basePrice),
      );
    }
  ...
  const bannerText = (() => {
    switch (special.type) {
      ...
      case 'conditional_add_on_price':
        return `🔓 Unlocks add-on @ R${special.conditions.overridePrice ?? 0}`;
      default:
        return null;
    }
  })();
  ...
  const bannerVariant =
    special.type === 'bundle'                   ? 'purple' :
    special.type === 'conditional_add_on_price' ? 'amber'  : 'blue';
  ...
  const handleAddToCart = (e: any) => {
    ...
    if (isAddonDeal && addonProduct) {
      setShowAddonModal(true);
    } else {
      Alert.alert(
        'Added to Cart',
        isMultibuy
          ? `${bundleQty} bundle${bundleQty > 1 ? 's' : ''} (${actualQty} items) added to your cart`
          : `${actualQty} × ${selectedVariant?.name || product.name} added to your cart`,
      );
  ...
  return (
    <>
        ...
        <View style={shared.content}>
          ...
          {hasVariants && (
            <View onStartShouldSetResponder={() => true}>
              <VariantPickerModal
                ...
                value={selectedVariant?._id ?? selectedVariantId}
          ...
          {bannerText && (
            bannerVariant === 'purple' ? (
              ...
              <View style={cardStyles.infoBannerAmber}>
                <Text style={cardStyles.infoBannerTextAmber} numberOfLines={2}>{bannerText}</Text>
              </View>
            ) : (
          ...
          <View style={shared.priceContainer}>
            ...
            {savings > 0 && special.type !== 'buy_x_get_y' && (
              <Text style={shared.oldPrice}>R{basePrice.toFixed(2)}</Text>
            )}
          </View>
          ...
          {inStock ? (
            <View style={shared.cartActions} onStartShouldSetResponder={() => true}>
              <View style={shared.quantityControl}>
                <TouchableOpacity
                  style={shared.quantityButton}
                  onPress={decrement}
                  disabled={isMultibuy ? bundleQty <= 1 : quantity <= 1}
                >
                  <Minus
                    color={(isMultibuy ? bundleQty <= 1 : quantity <= 1) ? '#d1d5db' : '#6b7280'}
                    size={16}
                  />
                </TouchableOpacity>
                ...
                >
                  <Plus
                    color={(isMultibuy ? (bundleQty + 1) * bundleSize > stock : quantity >= stock) ? '#d1d5db' : '#6b7280'}
                    size={16}
                  />
                </TouchableOpacity>
              </View>
      ...
      <Modal
        ...
        animationType="slide"
        statusBarTranslucent
        onRequestClose={handleDeclineAddon}
      >
        <TouchableWithoutFeedback onPress={handleDeclineAddon}>
          <View style={cardStyles.modalBackdrop} />
        </TouchableWithoutFeedback>
        <View style={cardStyles.modalSheet}>
          <View style={cardStyles.modalHeader}>
            <View>
              <Text style={cardStyles.modalEyebrow}>🔓 Special Unlocked!</Text>
              <Text style={cardStyles.modalTitle}>Would you like to add this?</Text>
            </View>
            <TouchableOpacity onPress={handleDeclineAddon}>
              <XCircle color="#9ca3af" size={22} />
            </TouchableOpacity>
          </View>

          <View style={cardStyles.modalBody}>
            {addonProduct?.images?.[0] ? (
              <Image source={{ uri: addonProduct.images[0] }} style={cardStyles.addonImage} />
            ) : (
              <View style={[cardStyles.addonImage, cardStyles.addonImagePlaceholder]}>
                <Tag color="#f59e0b" size={24} />
              </View>
            )}
            <View style={cardStyles.addonInfo}>
              ...
              <View style={cardStyles.addonPriceRow}>
                <Text style={cardStyles.addonPrice}>
                  R{(special.conditions.overridePrice ?? 0).toFixed(2)}
                </Text>
                {addonProduct && addonProduct.price > (special.conditions.overridePrice ?? 0) && (
                  <Text style={cardStyles.addonOldPrice}>R{addonProduct.price.toFixed(2)}</Text>
                )}
              </View>
              {addonProduct && addonProduct.price > (special.conditions.overridePrice ?? 0) && (
                <Text style={cardStyles.addonSavings}>
                  Save R{(addonProduct.price - (special.conditions.overridePrice ?? 0)).toFixed(2)}!
                </Text>
              )}
            </View>
          </View>
          ...
          <View style={cardStyles.modalActions}>
            <TouchableOpacity style={cardStyles.acceptBtn} onPress={handleAcceptAddon}>
              <CheckCircle color="#fff" size={16} />
              <Text style={cardStyles.acceptBtnText}>
                Yes, add for R{(special.conditions.overridePrice ?? 0).toFixed(2)}!
              </Text>
            </TouchableOpacity>
            <TouchableOpacity style={cardStyles.declineBtn} onPress={handleDeclineAddon}>
              <Text style={cardStyles.declineBtnText}>No thanks</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
...
const cardStyles = StyleSheet.create({
  infoBannerAmber: {
    ...
    borderRadius: 6, paddingHorizontal: 8, paddingVertical: 6, marginBottom: 6,
  },
  ...
  acceptBtn: {
    ...
    backgroundColor: '#f97316', borderRadius: 14, paddingVertical: 14,
  },
