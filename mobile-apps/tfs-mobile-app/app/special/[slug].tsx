...
export default function SpecialDetailScreen() {
  ...
  const loadSpecial = async () => {
    try {
      ...
      if (specialRes.data.specials && specialRes.data.specials.length > 0) {
        const spec = specialRes.data.specials[0];
        setSpecial(spec);
        
        // Load associated products based on special type
        let productIds: string[] = [];
        
        if (spec.type === 'buy_x_get_y') {
          // ✅ FIX: For buy_x_get_y, load the BUY product for pricing
          if (spec.conditions.buyProductId) {
            productIds = [spec.conditions.buyProductId];
          }
        } else if (spec.type === 'bundle') {
          // For bundles, load all bundle products
          if (spec.conditions.bundleProducts) {
            productIds = spec.conditions.bundleProducts.map((bp: any) => bp.productId);
          }
        } else {
          // For other types, use productId or productIds
          productIds = spec.productIds || (spec.productId ? [spec.productId] : []);
        }
        
        if (productIds.length > 0) {
          const fetchPromises = productIds.map((id: string) => 
            api.get(`/api/products/${id}`).then(res => res.data.product).catch(() => null)
          );
  ...
  const getSpecialPrice = (product: Product) => {
    ...
    switch (special.type) {
      case 'percentage_off':
        const percentOff = (product.price * (special.conditions?.discountPercentage || 0)) / 100;
        const maxDiscount = special.conditions?.maximumDiscount || Infinity;
        return product.price - Math.min(percentOff, maxDiscount);
      ...
      case 'multibuy':
        // Price per unit in multibuy
        return (special.conditions?.specialPrice || product.price) / (special.conditions?.requiredQuantity || 1);
      
      case 'buy_x_get_y':
        // ✅ FIX: Show the BUY product's regular price - the GET product is what's free/discounted
        return product.price;
      
      case 'bundle':
        // Bundle shows total bundle price, not per-item
  ...
  const savings = selectedProduct && special.type !== 'buy_x_get_y' 
  ...
  const displayImages = special.images && special.images.length > 0 
    ? special.images 
                ...
                {products.map((product) => (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      selectedProduct?._id === product._id && styles.productOptionActive,
                      product.stockLevel === 0 && styles.productOptionDisabled,
                    ]}
                  ...
                  >
                    ...
                    {product.stockLevel === 0 && (
                      <Text style={styles.productOptionOutOfStock}>Out of stock</Text>
                    )}
                  </TouchableOpacity>
                ))}
              </View>
          ...
          {selectedProduct && (
            <View style={styles.priceSection}>
              <View>
                ...
                {savings > 0 && (
                  <Text style={styles.oldPrice}>R{selectedProduct.price.toFixed(2)}</Text>
                )}
              </View>
              {savings > 0 && (
              ...
              {isInStock ? (
                <View style={styles.inStockRow}>
                  ...
                  <Text style={styles.inStockText}>
                    In Stock ({selectedProduct.stockLevel} available)
                  </Text>
                </View>
              ) : (
