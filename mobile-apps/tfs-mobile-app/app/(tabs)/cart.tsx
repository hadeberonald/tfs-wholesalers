...
export default function CartScreen() {
  ...
  const setSpecials      = useStore((state) => state.setSpecials);
  const getTotal         = useStore((state) => state.getTotal);
  const getSubtotal      = useStore((state) => state.getSubtotal);
  const getTotalSavings  = useStore((state) => state.getTotalSavings);
  ...
  const fetchSpecials = async () => {
    ...
    setLoading(true);
    try {
      const response = await api.get('/api/specials', {
        params: { branchId: branch._id, active: true },
      });
  ...
  const regularItems    = cart.filter((i) => !i.autoAdded && !i.isCombo);
  const comboItems      = cart.filter((i) => !i.autoAdded && i.isCombo);
  const freeItems       = cart.filter((i) => i.autoAdded && i.isFreeItem);
  const multibuyBonuses = cart.filter((i) => i.autoAdded && i.isMultibuyBonus);
  ...
  const handleRemoveItem = (id: string, variantId?: string) => {
    ...
    Alert.alert('Remove Item', 'Remove this item from your cart?', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Remove', style: 'destructive', onPress: () => removeFromCart(id, variantId) },
    ]);
  };
            ...
            <View key={`${item.id}-${item.variantId || ''}`} style={styles.cartItem}>
              ...
              <View style={styles.itemDetails}>
                ...
                <View style={styles.quantityControl}>
                  <TouchableOpacity style={styles.quantityButton} onPress={() => decreaseQuantity(item.id, item.quantity, item.variantId)}>
                    <Minus color="#6b7280" size={16} />
                  </TouchableOpacity>
                  <Text style={styles.quantityText}>{item.quantity}</Text>
                  <TouchableOpacity style={styles.quantityButton} onPress={() => increaseQuantity(item.id, item.quantity, item.variantId)}>
                    <Plus color="#6b7280" size={16} />
                  </TouchableOpacity>
                </View>
              </View>
              <View style={styles.itemActions}>
                <TouchableOpacity onPress={() => handleRemoveItem(item.id, item.variantId)} style={styles.removeButton}>
                    ...
                    <View style={styles.autoAddedHeader}>
                      ...
                      {item.comboItemCount && (
                        <View style={styles.comboItemCountBadge}>
                          <Text style={styles.comboItemCountText}>{item.comboItemCount} items</Text>
                        </View>
                      )}
                    </View>
      ...
      <View style={styles.summary}>
        ...
        {totalSavings > 0 && (
          <View style={styles.savingsRow}>
            ...
            <Text style={styles.savingsValue}>-R{totalSavings.toFixed(2)}</Text>
          </View>
        )}
        <View style={styles.summaryRow}>
        ...
        {totalSavings > 0 && (
          <View style={styles.celebrationBadge}>
            <Text style={styles.celebrationText}>🎉 You're saving R{totalSavings.toFixed(2)} with specials!</Text>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  itemActions: { justifyContent: 'center', alignItems: 'center', marginLeft: 8 },
  ...
  comboCount: {
    backgroundColor: '#7c3aed', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12,
  },
  ...
  bonusItemImage: {
    width: 80, height: 80, borderRadius: 8, backgroundColor: '#fff',
    borderWidth: 2, borderColor: '#ddd6fe',
  },
  comboBadge: {
    position: 'absolute', top: 4, right: 4, backgroundColor: '#7c3aed',
  ...
  discountBadgeInline: {
    backgroundColor: '#ef4444', paddingHorizontal: 5, paddingVertical: 2, borderRadius: 4,
  },
  ...
  // Bonus Items section (Buy X Get Y)
  ...
  bonusCount: {
    backgroundColor: '#10b981', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12,
  },
  ...
  freeBadge: {
    position: 'absolute', top: 4, right: 4, backgroundColor: '#10b981',
  ...
  infoBox: {
    flexDirection: 'row', backgroundColor: '#dbeafe', padding: 12,
    borderRadius: 8, gap: 8, borderWidth: 1, borderColor: '#93c5fd',
  },
  ...
  checkoutButton: {
    backgroundColor: '#FF6B35', paddingVertical: 16, borderRadius: 12,
