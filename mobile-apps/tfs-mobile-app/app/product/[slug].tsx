  ...
  const loadProduct = async () => {
    try {
      ...
      if (res.data.products?.length > 0) {
        ...
        setProduct(prod);
        // Do NOT auto-select a variant — base product is the default
      }
    } catch (error) {
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.infoSection}>
          ...
          {/* Stock — only show when low (under 10), otherwise just show Out of Stock */}
          {lowStock && (
            <View style={styles.stockWarning}>
              <Text style={styles.stockWarningText}>Only {stock} left in stock!</Text>
            </View>
          )}

          {/* Variant selector — base product is default */}
          {product.hasVariants && product.variants && product.variants.length > 0 && (
            <View style={styles.variantsSection}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                {/* Base product chip */}
                <TouchableOpacity
                  style={[styles.variantButton, !selectedVariant && styles.variantButtonActive]}
                  onPress={() => { setSelectedVariant(undefined); setQuantity(1); setActiveImageIndex(0); }}
                >
                  <Text style={[styles.variantButtonText, !selectedVariant && styles.variantButtonTextActive]}>
                    {product.name}
                  </Text>
                </TouchableOpacity>

                {/* Variant chips */}
                {product.variants.filter((v: ProductVariant) => v.active).map((variant: ProductVariant) => (
                  <TouchableOpacity
                    key={variant._id}
          ...
          <View style={styles.detailsSection}>
            ...
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailValue}>{selectedVariant?.sku || product.sku}</Text>
            </View>
            {(selectedVariant?.barcode || product.barcode) && (
              <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>Barcode:</Text>
                <Text style={styles.detailValue}>{selectedVariant?.barcode || product.barcode}</Text>
              </View>
            )}
            {product.unitQuantity && product.unit && (
              <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>Size:</Text>
                <Text style={styles.detailValue}>{product.unitQuantity}{product.unit}</Text>
              </View>
            )}
          </View>
        </View>
      ...
      <View style={[styles.bottomBar, { paddingBottom: insets.bottom || 16 }]}>
        <View style={styles.quantityControl}>
          ...
          >
            <Minus color={quantity <= 1 || !inStock ? '#d1d5db' : '#6b7280'} size={20} />
          </TouchableOpacity>
          <Text style={styles.quantityText}>{quantity}</Text>
          <TouchableOpacity
            style={styles.quantityButton}
            onPress={() => quantity < stock && setQuantity(quantity + 1)}
            disabled={quantity >= stock || !inStock}
          >
            <Plus color={quantity >= stock || !inStock ? '#d1d5db' : '#6b7280'} size={20} />
          </TouchableOpacity>
        </View>
        <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  specialBadge: {
    ...
    backgroundColor: '#FF6B35', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8, gap: 4,
  },
  ...
  variantButton: {
    backgroundColor: '#f3f4f6', paddingHorizontal: 16, paddingVertical: 12,
    borderRadius: 8, marginRight: 8, borderWidth: 2, borderColor: 'transparent',
  },
  ...
  variantOutOfStock: { fontSize: 12, color: '#ef4444' },
  ...
  quantityText: {
    fontSize: 18, fontWeight: '600', color: '#1f2937',
    marginHorizontal: 16, minWidth: 30, textAlign: 'center',
  },
  addToCartButton: {
    flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    backgroundColor: '#FF6B35', paddingVertical: 16, borderRadius: 12, gap: 8,
  },
