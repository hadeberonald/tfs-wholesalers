...
// Shows orders that are ready for driver collection, scoped to the active branch.
// Status flow expected: collecting → out_for_delivery → delivered
...
export default function DeliveriesListScreen({ navigation }: any) {
  ...
  const fetchDeliveries = async (silent = false) => {
    try {
      ...
      const deliveryOrders = response.data.orders.filter((order: Order) =>
    ...
    return (
      <TouchableOpacity style={styles.card} onPress={() => handleOrderPress(item)}>
          ...
          <View style={styles.infoRow}>
            ...
            <Text style={styles.infoText}>
              {packageCount > 0
                ...
                : `${item.items?.length || 0} items (not yet packaged)`}
            </Text>
          </View>
        ...
        <View style={styles.cardFooter}>
          ...
          <TouchableOpacity style={styles.actionBtn} onPress={() => handleOrderPress(item)}>
            <Truck size={16} color="#fff" />
            <Text style={styles.actionBtnText}>
              {item.status === 'collecting' ? 'Collect' : 'View'}
            </Text>
          </TouchableOpacity>
        </View>
      </TouchableOpacity>
    ...
    <View style={styles.container}>
      ...
      {orders.length === 0 ? (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyText}>No orders ready for collection right now.</Text>
