...
// Alert.alert replaced with useAppModal throughout.
// Navigation after branch select uses navigation.reset() to land on Main
// and clear BranchSelect off the stack.
...
export default function BranchSelectScreen() {
    ...
    try {
      ...
      if (!silent) {
        showModal({
          ...
          message: 'Could not load branches. Please check your connection.',
          buttons: [{ text: 'OK' }],
        });
      }
    } finally {
  ...
  const handleSelectBranch = async (branch: Branch) => {
    ...
    try {
      ...
      // Reset the stack so Main is root — BranchSelect is gone from history.
      // This works whether we arrived here as the initial screen or as a modal.
  ...
  const renderBranch = ({ item }: { item: Branch }) => {
    ...
    return (
      <TouchableOpacity
        style={[styles.branchCard, isCurrentBranch && styles.branchCardActive]}
        onPress={() => handleSelectBranch(item)}
        disabled={!!selecting}
        activeOpacity={0.7}
      >
        <View style={[styles.branchIconWrap, isCurrentBranch && styles.branchIconWrapActive]}>
          <Building2 size={24} color={isCurrentBranch ? '#fff' : '#FF6B35'} />
        </View>
...
const styles = StyleSheet.create({
  ...
  descRow: {
    ...
    backgroundColor: '#fff7f3', paddingHorizontal: 20, paddingVertical: 12,
  ...
  branchCard: {
    ...
    borderWidth: 2, borderColor: '#e5e7eb', gap: 14,
    shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 8, elevation: 2,
  },
  branchCardActive: {
    backgroundColor: '#FF6B35', borderColor: '#FF6B35',
    shadowColor: '#FF6B35', shadowOpacity: 0.35, shadowRadius: 12, elevation: 6,
  },

  branchIconWrap: {
    width: 48, height: 48, borderRadius: 24,
    backgroundColor: '#fff7f3', alignItems: 'center', justifyContent: 'center',
  ...
  footerNote: { textAlign: 'center', fontSize: 12, color: '#9ca3af', marginTop: 8, marginBottom: 16 },
});