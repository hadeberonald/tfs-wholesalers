...
import {
  Package, QrCode, CheckCircle, Gift, Tag, Layers, ShoppingBag, ChevronDown, ChevronUp,
...
export default function PackagingScreen({ navigation: navProp }: any) {
  ...
  const handleQRScanned = async ({ data }: any) => {
    ...
    const newPackage      = { qrCode: data, packageNumber: packageNum, totalPackages: total, items: Array.from(currentPackageItems) };
        ...
        <View style={styles.section}>
          ...
          {scanning ? (
            ...
            >
              ...
              <Text style={styles.scanButtonText}>Scan QR Label — Package {packages.length + 1}</Text>
            </TouchableOpacity>
          )}
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  scanButton:         { backgroundColor: '#FF6B35', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 16, borderRadius: 12, marginTop: 16, gap: 8 },
