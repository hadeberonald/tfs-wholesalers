  ...
  return (
    <View style={[gs.wrapper, { borderColor: group.borderColor }]}>
      ...
      {expanded && group.items.map((item, idx) => (
        <ItemCard key={scanKeys[idx]} item={item} scanKey={scanKeys[idx]}
          pickCount={pickCounts[scanKeys[idx]] ?? 0} isOOS={oosSet.has(scanKeys[idx])}
          onManual={onManual} onOOS={onOOS} groupColor={group.color} groupBg={group.bgColor} isScanModeActive={isScanModeActive} />
      ))}
    </View>
  );
}
...
export default function PickingScreen({ route, navigation: navProp }: any) {
  ...
  const handleBarcodeScannedWithFallback = useCallback(async (scannedBarcode: string) => {
    ...
    if (!allBarcodes.includes(scannedBarcode)) {
      try { const res = await axios.get(`${API_URL}/api/products?barcode=${scannedBarcode}`); const productName = res.data.product?.name || 'Unknown product'; showModal({ title: '✗ Not in Order', message: `Scanned: ${productName}\n\nThis barcode is not part of this order.`, buttons: [{ text: 'OK' }] }); }
  ...
  }, [handleBarcodeScanned, showModal]);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={[styles.scanToggle, scannerVisible && styles.scanToggleActive]} onPress={() => setScannerVisible(v => !v)}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {groups.map((grp, gi) => (
          <GroupSection key={grp.key} group={grp} scanKeys={groupScanKeys[gi]} pickCounts={pickCounts} oosSet={oos} onManual={handleManual} onOOS={handleOOS} isScanModeActive={scannerVisible} />
        ))}
    ...
    try {
      ...
      if (item.comboItems && item.comboItems.length > 0) {
        enrichedComboItems = await Promise.all(item.comboItems.map(async (ci) => {
          try { const cr = await axios.get(`${API_URL}/api/products/${ci.productId}`, { headers: { Authorization: `Bearer ${token}` } }); const cp = cr.data.product || cr.data; return { ...ci, barcode: cp.barcode || ci.barcode, image: cp.images?.[0] || ci.image, sku: cp.sku || ci.sku }; }
          catch { return ci; }
        }));
      }
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#f5f5f5' }, centered: { flex: 1, justifyContent: 'center', alignItems: 'center', gap: 12 },
  loadingText: { fontSize: 15, color: '#666' }, errorText: { fontSize: 16, color: '#999', marginTop: 12 },
  ...
  backPress: { paddingRight: 8 }, backText: { fontSize: 16, color: '#FF6B35', fontWeight: '600' },
  headerTitle: { fontSize: 22, fontWeight: 'bold', color: '#1a1a1a' }, customerName: { fontSize: 13, color: '#666', marginTop: 2 },
  backBtn: { marginTop: 12, paddingHorizontal: 20, paddingVertical: 10, borderRadius: 10, borderWidth: 2, borderColor: '#FF6B35' }, backBtnText: { color: '#FF6B35', fontWeight: '600' },
  ...
  scanToggleActive: { backgroundColor: '#FF6B35', borderColor: '#FF6B35' }, scanToggleText: { fontSize: 14, fontWeight: '700', color: '#FF6B35' }, scanToggleTextActive: { color: '#fff' },
  ...
  progressFill: { height: '100%', backgroundColor: '#10b981', borderRadius: 5 }, progressFillOOS: { height: '100%', backgroundColor: '#ef4444' },
  chipRow: { flexDirection: 'row', gap: 8, flexWrap: 'wrap' }, chip: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20, borderWidth: 1 }, chipText: { fontSize: 11, fontWeight: '700' },
...
const gs = StyleSheet.create({
  ...
  iconWrap: { width: 40, height: 40, borderRadius: 20, alignItems: 'center', justifyContent: 'center' }, title: { fontSize: 15, fontWeight: '800' }, subtitle: { fontSize: 12, color: '#6b7280', marginTop: 2 },
  progress: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20, backgroundColor: '#e5e7eb', marginRight: 8 }, progressDone: { backgroundColor: '#d1fae5' }, progressText: { fontSize: 12, fontWeight: '700', color: '#374151' }, progressTextDone: { color: '#065f46' },
});
const ic = StyleSheet.create({
  card: { backgroundColor: '#fff', borderWidth: 2, borderRadius: 12, padding: 14, margin: 8, marginTop: 0, position: 'relative' }, cardScanned: { backgroundColor: '#f0fdf4', borderColor: '#10b981' }, cardOOS: { backgroundColor: '#fff5f5', borderColor: '#ef4444' },
  scannedBanner: { position: 'absolute', top: 10, right: 10, flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#10b981', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 20, zIndex: 1 }, scannedBannerText: { color: '#fff', fontSize: 10, fontWeight: '800' },
  oosBanner: { position: 'absolute', top: 10, right: 10, flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#ef4444', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 20, zIndex: 1 }, oosBannerText: { color: '#fff', fontSize: 10, fontWeight: '800' },
  partialBanner: { position: 'absolute', top: 10, right: 10, backgroundColor: '#f59e0b', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 20, zIndex: 1 }, partialBannerText: { color: '#fff', fontSize: 10, fontWeight: '800' },
  lowStockBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#fef3c7', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, marginBottom: 6 }, lowStockText: { color: '#92400e', fontSize: 11, fontWeight: '600' },
  row: { flexDirection: 'row', gap: 12 }, img: { width: 70, height: 70, borderRadius: 10, backgroundColor: '#f3f4f6' }, imgFb: { alignItems: 'center', justifyContent: 'center' }, info: { flex: 1 },
  name: { fontSize: 14, fontWeight: '700', color: '#1a1a1a', lineHeight: 20 }, variant: { fontWeight: '400', color: '#6b7280' }, sku: { fontSize: 11, color: '#9ca3af', marginTop: 2, marginBottom: 4 },
  badge: { borderRadius: 6, paddingHorizontal: 8, paddingVertical: 5, marginBottom: 6 }, badgeText: { fontSize: 11, fontWeight: '800', letterSpacing: 0.3 }, badgeDesc: { fontSize: 11, fontWeight: '500', marginTop: 1 },
  priceRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 4 }, qtyPill: { backgroundColor: '#fff7ed', borderRadius: 6, paddingHorizontal: 7, paddingVertical: 3 }, qtyPillPartial: { backgroundColor: '#fef3c7' }, qtyPillDone: { backgroundColor: '#d1fae5' },
  qtyText: { color: '#FF6B35', fontSize: 12, fontWeight: '700' }, qtyTextPartial: { color: '#92400e' }, qtyTextDone: { color: '#065f46' },
  origPrice: { fontSize: 11, color: '#9ca3af', textDecorationLine: 'line-through' }, price: { fontSize: 14, fontWeight: '700', color: '#FF6B35' }, priceFree: { color: '#10b981' },
  barcodeCol: { width: 28, alignItems: 'center', justifyContent: 'flex-start', paddingTop: 2 }, barcodeHas: { width: 28, height: 28, borderRadius: 14, backgroundColor: '#d1fae5', alignItems: 'center', justifyContent: 'center' }, barcodeNone: { width: 28, height: 28, borderRadius: 14, backgroundColor: '#fef3c7', alignItems: 'center', justifyContent: 'center' },
  actions: { flexDirection: 'row', gap: 8, marginTop: 12, paddingTop: 10, borderTopWidth: 1, borderTopColor: '#f3f4f6' },
  scanHint: { flex: 2, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: '#fff7ed', padding: 12, borderRadius: 10, borderWidth: 1, borderColor: '#fed7aa' }, scanHintActive: { backgroundColor: '#FF6B35', borderColor: '#FF6B35' },
  scanHintText: { color: '#FF6B35', fontSize: 12, fontWeight: '600', flexShrink: 1 }, scanHintTextActive: { color: '#fff' },
  manualBtn: { flex: 1, flexDirection: 'row', backgroundColor: '#374151', padding: 12, borderRadius: 10, alignItems: 'center', justifyContent: 'center', gap: 6 }, manualBtnText: { color: '#fff', fontWeight: '700', fontSize: 14 },
  oosBtn: { width: 48, backgroundColor: '#ef4444', padding: 12, borderRadius: 10, alignItems: 'center', justifyContent: 'center' },
  noBarcodeWarn: { flex: 2, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: '#fffbeb', padding: 12, borderRadius: 10, borderWidth: 1, borderColor: '#fde68a' }, noBarcodeText: { color: '#92400e', fontSize: 12, fontWeight: '600' },
  oosNote: { flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 10, padding: 10, backgroundColor: '#fee2e2', borderRadius: 8 }, oosNoteText: { flex: 1, fontSize: 12, color: '#991b1b', fontWeight: '500' },
});