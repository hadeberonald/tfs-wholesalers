 ...
 */
// _2 suffix = Ladysmith's own price card, if it ever differs from
// Dundee's. Meta's per-category pricing is usually the same across a
// company's WABAs in the same country, so this falls back to the
// unsuffixed value rather than requiring a duplicate set just to keep
// working out of the box.
const RATES = {
  marketing: parseFloat(process.env.WA_RATE_MARKETING_2 || process.env.WA_RATE_MARKETING || "0"),
  utility: parseFloat(process.env.WA_RATE_UTILITY_2 || process.env.WA_RATE_UTILITY || "0"),
  service: parseFloat(process.env.WA_RATE_SERVICE_2 || process.env.WA_RATE_SERVICE || "0"),
  authentication: parseFloat(
    process.env.WA_RATE_AUTHENTICATION_2 || process.env.WA_RATE_AUTHENTICATION || "0"
  ),
};
const RATE_CURRENCY = process.env.WA_RATE_CURRENCY_2 || process.env.WA_RATE_CURRENCY || "ZAR";
